...
export default function JournalDashboard({ activeJournalId }: JournalDashboardProps) {
  ...
  // Fetch active entry if selected
  ...
  // Sync title when active ID changes
  useEffect(() => {
    setLocalTitle(null);
  }, [activeJournalId]);
  ...
  return (
    <SidebarProvider className="landing-theme layout-locked h-dvh overflow-hidden bg-background text-on-background">
      ...
      <nav className="tablet:hidden fixed bottom-0 left-0 right-0 bg-surface/80 backdrop-blur-xl border-t border-black/5 px-[24px] py-[12px] flex justify-around items-center z-50">
        <button onClick={() => router.push("/write")} className="flex flex-col items-center gap-[4px] text-primary cursor-pointer">
          <span className="material-symbols-outlined">book_5</span>
          <span className="text-[10px] font-bold">Journal</span>
        </button>
        <button onClick={() => router.push("/chat")} className="flex flex-col items-center gap-[4px] text-secondary cursor-pointer">
          <span className="material-symbols-outlined">chat_bubble</span>
          <span className="text-[10px] font-bold">Chat</span>
        </button>
        <button onClick={() => router.push("/talk")} className="flex flex-col items-center gap-[4px] text-secondary cursor-pointer">
          <span className="material-symbols-outlined">mic</span>
          <span className="text-[10px] font-bold">Voice</span>
        </button>
        <button onClick={() => router.push("/onboarding")} className="flex flex-col items-center gap-[4px] text-secondary cursor-pointer">
          <span className="material-symbols-outlined">account_circle</span>
          <span className="text-[10px] font-bold">Profile</span>
        </button>
      </nav>

      {/* Mobile Floating Action Button (FAB) for new entries */}
      <NewJournalDialog>
        <button className="tablet:hidden fixed right-[24px] bottom-[80px] w-[56px] h-[56px] bg-primary text-on-primary rounded-full shadow-lg shadow-primary/30 flex items-center justify-center z-50 cursor-pointer hover:scale-105 active:scale-95 transition-transform">
